...
export default function Home({ userData, onLogout, onNavigateToManage }) {
  ...
  const handleAlertStatusUpdate = useCallback(async (alertId, status, reason) => {
    try {
      const body = { id: alertId, status };
      ...
      if (data.ret_code === 0) {
        // Update local viewedAlert status for instant UI feedback
        if (viewedAlert?.id === alertId) {
          setViewedAlert(prev => ({ ...prev, status }));
        }
        // Always mark as seen if confirmed
        setSeenAlertIds(prev => {
  ...
  }, [viewedAlert, setSeenAlertIds]);

  const viewedAlertIndex = useMemo(() => {
    if (!viewedAlert || !alerts.length) return -1;
    return alerts.findIndex(a => a.id === viewedAlert.id);
  }, [viewedAlert, alerts]);

  const handleNextAlert = useCallback(() => {
    if (viewedAlertIndex < alerts.length - 1) {
      const nextAlert = alerts[viewedAlertIndex + 1];
      handleAlertClick(nextAlert);
    }
  }, [viewedAlertIndex, alerts, handleAlertClick]);

  const handlePrevAlert = useCallback(() => {
    if (viewedAlertIndex > 0) {
      const prevAlert = alerts[viewedAlertIndex - 1];
      handleAlertClick(prevAlert);
    }
  }, [viewedAlertIndex, alerts, handleAlertClick]);
  ...
  return (
    <div className="fixed inset-0 bg-white overflow-hidden">
      ...
      <RightPanel
        ...
        alerts={alerts.filter(a => !seenIdSet.has(a.id))}
      ...
      <AlertImageDialog
        ...
        onPrev={handlePrevAlert}
        onStatusUpdate={handleAlertStatusUpdate}
        hasNext={viewedAlertIndex < alerts.length - 1}
        hasPrev={viewedAlertIndex > 0}
      />
