...
export default function Login({ onLoginSuccess }) {
    ...
    useEffect(() => {
        ...
        // Kiểm tra lý do bị đá (nếu có) từ App.jsx hoặc sessionStorage (bền vững qua F5)
        const hash = window.location.hash;
        const storedReason = sessionStorage.getItem("loginErrorReason");
        const reason = hash.includes('reason=') ? hash.split('reason=')[1] : (storedReason || "");

        if (reason === 'kicked' || reason === 'concurrent_login') {
            setError("Tài khoản của bạn vừa đăng nhập ở một thiết bị khác. Bạn đã bị đăng xuất để bảo mật.");
        } else if (reason === 'refresh_token_expired' || reason === 'expired') {
            setError("Phiên làm việc đã hết hạn. Vui lòng đăng nhập lại.");
        }

        // Xóa mã lỗi trong sessionStorage để không bị hiện lại sau khi đăng nhập thành công sau này
        if (storedReason) {
