...
import {
    ...
    IoNotificationsOutline, IoNotificationsOffOutline, IoHelpCircleOutline
...
export default function ManageEvents({ onClose }) {
    ...
    const [aiConfigKey, setAiConfigKey] = useState(Date.now()); // Thêm key để force refresh iframe
    ...
    const handleRefreshAiConfig = () => {
        setAiConfigKey(Date.now());
    };
        ...
        <div className="fixed inset-0 bg-slate-50 overflow-hidden flex z-50 font-sans">
                ...
                <nav className="flex-1 py-4 overflow-y-auto px-3 space-y-1">
                    ...
                    <button
                        onClick={() => {
                            ...
                            setIsSidebarOpen(false);
                            setAiConfigKey(Date.now()); // Force refresh khi vào tab
                        }}
                        className={`
                            w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all
                            ${activeTab === 'ai_config'
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-white shadow-inner relative">
                ...
                <div className="flex items-center justify-between px-3 py-3 border-b border-slate-200 bg-white h-14 flex-shrink-0">
                    ...
                    {activeTab === 'ai_config' && (
                        <button
                            onClick={handleRefreshAiConfig}
                            className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-blue-600 rounded-full border border-blue-100 hover:bg-blue-100 transition-all text-xs font-bold"
                        >
                            <IoRefreshOutline size={14} className={activeTab === 'ai_config' ? "" : ""} />
                            Làm mới cấu hình
                        </button>
                    )}
                </div>
                ...
                {activeTab === 'events' ? (
                                            ...
                                            <div className="space-y-4">
                                                    ...
                                                    !selectedEvent.status && (
                                                        <div className="pt-4 border-t border-slate-100">
                                                            ...
                                                            <button
                                                                onClick={async () => {
                                                                    setDownloading(true);
                                                                    const res = await updateEventStatus(selectedEvent.id, 'unclear');
                                                                    if (res.success) {
                                                                        setSelectedEvent(prev => ({ ...prev, status: 'unclear' }));
                                                                    }
                                                                    setDownloading(false);
                                                                }}
                                                                disabled={downloading}
                                                                className="mt-3 w-full flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-sm transition-all border-2 bg-white text-orange-600 border-orange-600 hover:bg-orange-50"
                                                            >
                                                ...
                                                {selectedEvent.status && (
                                                    <div className="pt-4 border-t border-slate-100">
                                                        ...
                                                        `}>
                                                            {selectedEvent.status === 'true' ? <IoCheckmarkCircleOutline size={18} /> : (selectedEvent.status === 'unclear' ? <IoHelpCircleOutline size={18} /> : <IoClose size={18} />)}
                                                            {selectedEvent.status === 'true' 
                                                                ? t('manageEvents.feedback.correct') 
                                                                : (selectedEvent.status === 'unclear' 
                                                                    ? t('manageEvents.feedback.unclear') 
                                                                    : t('manageEvents.feedback.incorrect'))}
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex items-center justify-center gap-2">
                                                        ...
                                                        <button
                                                            onClick={() => {
                                                                ...
                                                                setActiveTab('ai_config');
                                                                setIsSidebarOpen(false);
                                                                setAiConfigKey(Date.now()); // Force refresh khi chọn camera
                                                            }}
                    ...
                    <div className="flex-1 overflow-hidden relative bg-slate-100 p-4">
                        <div className="w-full h-full bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden relative">
                            <iframe
                                key={aiConfigKey}
                                src={`ai_config.html?v=${aiConfigKey}&api_base=${encodeURIComponent(currentConfig.apiBase)}&camera_code=${encodeURIComponent(configuringAi?.code || '')}`}
