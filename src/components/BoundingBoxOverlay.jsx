...
export default function BoundingBoxOverlay({ boxes, imgDims }) {
    ...
    // Use defaults if dims are missing to avoid division by zero but keep component in DOM
    const dw = imgDims?.w || 1920;
    const dh = imgDims?.h || 1080;

    return (
        <div className="absolute inset-0 pointer-events-none z-30" data-box-count={safeBoxes.length}>
            {safeBoxes.map((box, idx) => {
                ...
                const left = (box.x1 / dw) * 100;
                const top = (box.y1 / dh) * 100;
                const width = ((box.x2 - box.x1) / dw) * 100;
                const height = ((box.y2 - box.y1) / dh) * 100;
