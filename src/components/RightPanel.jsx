...
import { IoCheckmarkCircleOutline, IoClose, IoHelpCircleOutline } from "react-icons/io5";
import { currentConfig } from "../config/factoryConfig";
import BoundingBoxOverlay from "./BoundingBoxOverlay";

// Small sub-component to handle local image dimensions for bounding box overlay
function AlertPreview({ imageUrl, eventCode, boxes }) {
  const [imgDims, setImgDims] = useState({ w: 0, h: 0 });

  return (
    <div className="w-full h-40 bg-black/70 relative flex items-center justify-center overflow-hidden">
      <div className="relative inline-block">
        ...
        {boxes && (
          <BoundingBoxOverlay
            boxes={typeof boxes === 'string' ? JSON.parse(boxes) : boxes}
            imgDims={imgDims}
          />
        )}
      </div>
...
export default function RightPanel({
  ...
  // { alertId, reason }
  const [incorrectInput, setIncorrectInput] = useState(null);
            ...
            return (
              <div
                ...
                className={`
                  ...
                  flex flex-col overflow-hidden transition group
                  ${isSeen
              ...
              >
                ...
                {/* Direct Action Buttons - Added below preview */}
                <div className="px-2 pb-2 flex gap-1.5 border-t border-slate-100 bg-white/50 pt-2">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      onAlertStatusUpdate && onAlertStatusUpdate(alert.id, 'true');
                    }}
                    className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg bg-green-50 text-green-600 hover:bg-green-100 transition-colors text-[10px] font-bold"
                    title={t("manageEvents.feedback.correct")}
                  >
                    <IoCheckmarkCircleOutline size={14} />
                    {t("manageEvents.feedback.correct") || "Đúng"}
                  </button>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      // Show reason input instead of immediately updating
                      setIncorrectInput({ alertId: alert.id, reason: '' });
                    }}
                    className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition-colors text-[10px] font-bold"
                    title={t("manageEvents.feedback.incorrect")}
                  >
                    <IoClose size={14} />
                    {t("manageEvents.feedback.incorrect") || "Sai"}
                  </button>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      onAlertStatusUpdate && onAlertStatusUpdate(alert.id, 'unclear');
                    }}
                    className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg bg-orange-50 text-orange-600 hover:bg-orange-100 transition-colors text-[10px] font-bold"
                    title={t("manageEvents.feedback.unclear")}
                  >
                    <IoHelpCircleOutline size={14} />
                    {t("manageEvents.feedback.unclear") || "Không rõ"}
                  </button>
                </div>

                {/* INCORRECT REASON INPUT - hiện khi click Sai */}
                {incorrectInput?.alertId === alert.id && (
                  <div
                    className="px-2 pb-2 pt-1 bg-red-50/80 border-t border-red-100 space-y-1.5"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <p className="text-[10px] font-bold text-red-500 uppercase tracking-wide">
                      {t("manageEvents.feedback.incorrectReasonLabel") || "Lý do sai ?"}
                    </p>
                    <textarea
                      autoFocus
                      rows={2}
                      value={incorrectInput.reason}
                      onChange={(e) =>
                        setIncorrectInput((prev) => ({ ...prev, reason: e.target.value }))
                      }
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' && !e.shiftKey) {
                          e.preventDefault();
                          if (incorrectInput.reason.trim()) {
                            onAlertStatusUpdate && onAlertStatusUpdate(alert.id, 'fail', incorrectInput.reason.trim());
                            setIncorrectInput(null);
                          }
                        }
                        if (e.key === 'Escape') setIncorrectInput(null);
                      }}
                      placeholder={t("manageEvents.feedback.incorrectReasonPlaceholder") || "Nhập lý do... (Enter để xác nhận)"}
                      className="w-full rounded-md border border-red-200 bg-white px-2 py-1.5 text-[10px] text-slate-700 resize-none focus:outline-none focus:ring-2 focus:ring-red-400/40 focus:border-red-400 placeholder:text-slate-300"
                    />
                    <div className="flex gap-1.5">
                      <button
                        disabled={!incorrectInput.reason.trim()}
                        onClick={() => {
                          onAlertStatusUpdate && onAlertStatusUpdate(alert.id, 'fail', incorrectInput.reason.trim());
                          setIncorrectInput(null);
                        }}
                        className="flex-1 py-1.5 rounded-lg bg-red-500 text-white text-[10px] font-bold hover:bg-red-600 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                        {t("manageEvents.feedback.incorrectConfirm") || "Xác nhận"}
                      </button>
                      <button
                        onClick={() => setIncorrectInput(null)}
                        className="px-3 py-1.5 rounded-lg bg-slate-100 text-slate-500 text-[10px] font-bold hover:bg-slate-200 transition-colors"
                      >
                        {t("manageEvents.feedback.incorrectCancel") || "Huỷ"}
                      </button>
                    </div>
                  </div>
                )}
