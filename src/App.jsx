...
export default function App() {
        ...
        fetch(`${currentConfig.apiBase}/heartbeat`, {
          ...
          .then(data => {
            ...
            if (data.status === 'ignored') {
              console.warn("Heartbeat ignored:", data.reason, data.message, data.debug);
              
              if (data.reason === 'session_mismatch') {
                console.warn("Attempting to heal session for:", userData.empno);
                ...
                if (storedUser) {
                    ...
                    refreshAccess(user.empno, user.refreshToken, fingerprint).then(result => {
                        if (result.allow) {
                            ...
                            console.log("Session healed successfully for:", user.empno);
                        } else {
                            console.error("Heal failed:", result.reason);
  ...
  }, [isAuthenticated, userData, fingerprint]);
  ...
  const handleLogout = async (notifyServer = true, reason = '') => {
    ...
    // Nếu bị kick hoặc hết hạn, chuyển hướng kèm mã lỗi và lưu vào sessionStorage để bền vững qua F5
    if (reason) {
      sessionStorage.setItem("loginErrorReason", reason);
