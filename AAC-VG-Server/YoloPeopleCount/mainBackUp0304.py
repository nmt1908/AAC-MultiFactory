...
async def lifespan(app: FastAPI):
    ...
    # 1) Preload ALL camera configs into memory
    ...
    # 5) Warm-up YOLO
    try:
        await asyncio.to_thread(warmup_yolo_models, 640, 10)
        init_dino_once()
    except Exception as e:
        print(f"[STARTUP] Warmup/DINO error (ignored): {e}")

    # 7) Start background AI processing loop
    loop_task = asyncio.create_task(new_background_loop())
    print("[STARTUP] Started background AI processing loop")

    # 8) Start Management loops (Batching & Health Check)
    asyncio.create_task(asyncio.to_thread(flush_alerts_loop))
    asyncio.create_task(asyncio.to_thread(check_camera_job))
    print("[STARTUP] Started Management loops (Batching & Health Check)")
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Use asterisk or CCTV_ORIGINS if defined
    allow_methods=["*"],
    allow_headers=["*"],
...
# ==========================
# CATEGORY & AREA MANAGEMENT APIs
# ==========================


@app.get("/api/cctv/filter-list")
async def get_filter_list():
    """
    Equivalent to cctvCtrl.php -> getListToFilter
    """
    try:
        conn = get_db_connection()
        with conn.cursor() as cursor:
            cursor.execute("SELECT DISTINCT location, code FROM cctv_tbl")
            locations = cursor.fetchall()

            cursor.execute("SELECT * FROM cctv_area_tbl")
            areas = cursor.fetchall()

            cursor.execute("SELECT * FROM cctv_category_tbl")
            categories = cursor.fetchall()
        conn.close()
        return {"locations": locations, "areas": areas, "category": categories}
    except Exception as e:
        return {"error": str(e)}


@app.get("/api/cctv/categories")
...
@app.post("/api/cctv/areas")
async def add_new_area(req: dict):
    """
    Equivalent to cctvCtrl.php -> addNewArea
    """
    area = req.get("area")
    if not area:
        return {"ret_code": -1, "msg": "area is required"}
    try:
        conn = get_db_connection()
        with conn.cursor() as cursor:
            cursor.execute(
                "INSERT INTO cctv_area_tbl (area) VALUES (%s)", (json.dumps(area),)
            )
            area_id = cursor.lastrowid
        conn.commit()
        conn.close()
        return {"ret_code": 0, "msg": "Created new area", "data": {"id": area_id}}
    except Exception as e:
        return {"ret_code": -1, "msg": str(e)}


@app.post("/api/cctv/purposes")
async def add_new_purpose(req: dict):
    """
    Equivalent to cctvCtrl.php -> addNewPurpose
    """
    purpose_code = req.get("purpose_code")
    purpose = req.get("purpose")
    if not purpose_code or not purpose:
        return {"ret_code": -1, "msg": "purpose_code and purpose are required"}
    try:
        conn = get_db_connection()
        with conn.cursor() as cursor:
            cursor.execute(
                "INSERT INTO cctv_category_tbl (purpose_code, category) VALUES (%s, %s)",
                (purpose_code, json.dumps(purpose)),
            )
            purpose_id = cursor.lastrowid
        conn.commit()
        conn.close()
        return {"ret_code": 0, "msg": "Created new purpose", "data": {"id": purpose_id}}
    except Exception as e:
        return {"ret_code": -1, "msg": str(e)}


@app.post("/api/cctv/filter-cameras")
async def filter_cameras(req: dict):
    """
    Equivalent to cctvCtrl.php -> filterDataWhenSelectedChanges
    """
    ...
    try:
        conn = get_db_connection()
        with conn.cursor() as cursor:
            sql = "SELECT code FROM cctv_tbl WHERE status = 'working'"
            params = []

            if location:
                # location is JSON, need to extract English name or match raw
                sql += " AND JSON_EXTRACT(location, '$.en') = %s"
                params.append(location)

            if area:
                # area column contains JSON array like ["1", "2"]
                sql += " AND JSON_CONTAINS(area, %s)"
                params.append(json.dumps(str(area)))

            if category:
                sql += " AND JSON_CONTAINS(category, %s)"
                params.append(json.dumps(str(category)))
        ...
        conn.close()
        return {"camerasSelected": codes}
    except Exception as e:
        return {"error": str(e)}


@app.post("/api/cctv/getManageWarnings")
async def get_manage_warnings(req: dict):
    """
    Equivalent to cctvCtrl.php -> getRecentWarnings (BUT with pagination and filters)
    Used by ManageEvents screen via useManageEvents hook.
    """
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            if camera_code:
                where_clauses.append("camera_code LIKE %s")
                params.append(f"%{camera_code}%")
            ...
            # 2. Get Total for Pagination
            count_sql = f"SELECT COUNT(*) as total FROM cctv_warning_tbl {where_str}"
            cursor.execute(count_sql, params)
            total = cursor.fetchone()["total"]

            # 3. Get Items
            sql = f"""
                SELECT id, camera_code, event_code, thumbshot_url, fullshot_url, boxes, created_at 
                FROM cctv_warning_tbl 
                {where_str}
            ...
            # Convert datetimes to unix or string
            for it in items:
                it["created_unix"] = int(it["created_at"].timestamp())
                it["created_at"] = it["created_at"].strftime("%Y-%m-%d %H:%M:%S")

            # 4. Get Counts by type (simplified for summary)
            cursor.execute(
                f"SELECT event_code, COUNT(*) as cnt FROM cctv_warning_tbl {where_str} GROUP BY event_code",
                params,
            )
            count_rows = cursor.fetchall()
            counts = {"all": total}
            for r in count_rows:
                counts[r["event_code"]] = r["cnt"]
...
STORAGE_PATH = os.getenv("STORAGE_PATH", "/mnt/cctv-map/storage/app/cctv")
...
# ==========================
# STORAGE & IMAGE APIs (MIGRATED FROM LARAVEL)
# ==========================


@app.get("/api/cctv/download")
async def download_img(photo: str):
    """
    Equivalent to cctvCtrl.php -> downloadImg
    """
    file_path = os.path.join(STORAGE_PATH, photo)
    if not os.path.exists(file_path):
        raise HTTPException(status_code=404, detail=f"File not found: {photo}")
    return FileResponse(file_path)


@app.post("/api/cctv/removePhoto")
async def remove_photo(req: dict):
    """
    Equivalent to cctvCtrl.php -> removePhoto
    """
    photo_data = req.get("photoData")
    if not photo_data:
        return {"ret_code": -1, "msg": "photoData is required"}

    file_path = os.path.join(STORAGE_PATH, photo_data)
    if os.path.exists(file_path):
        try:
            os.remove(file_path)
            return {"ret_code": 0, "msg": "File removed"}
        except Exception as e:
            return {"ret_code": -1, "msg": f"Failed to remove file: {e}"}
    else:
        return {"ret_code": -1, "msg": "File not found"}


@app.post("/api/cctv/ai/insert_warning")
async def insert_warning_from_avg(
    camera_code: str = Form(...),
    event_code: str = Form(...),
    boxes: str = Form(None),
    fullshot_url: UploadFile = File(None),
    thumbshot_url: UploadFile = File(None),
):
    """
    Equivalent to cctvCtrl.php -> insertWarningFromAVG
    Ported logic for saving fullshot and thumbshot into date-based folders.
    """
    date_str = time.strftime("%Y%m%d")
    base_dir = os.path.join(STORAGE_PATH, date_str)
    fullshot_dir = os.path.join(base_dir, "fullshot")
    thumbshot_dir = os.path.join(base_dir, "thumbshot")

    os.makedirs(fullshot_dir, exist_ok=True)
    os.makedirs(thumbshot_dir, exist_ok=True)
    ...
    if fullshot_url:
        ...
        save_path = os.path.join(fullshot_dir, filename)
        with open(save_path, "wb") as f:
            f.write(await fullshot_url.read())
        full_path_db = f"{date_str}/fullshot/{filename}"

    if thumbshot_url:
        filename = f"{camera_code}_{timestamp}_thumb.jpg"
        save_path = os.path.join(thumbshot_dir, filename)
        with open(save_path, "wb") as f:
            f.write(await thumbshot_url.read())
        thumb_path_db = f"{date_str}/thumbshot/{filename}"

    try:
        conn = get_db_connection()
        with conn.cursor() as cursor:
            sql = """
                INSERT INTO cctv_warning_tbl 
                (camera_code, event_code, thumbshot_url, fullshot_url, boxes, created_at)
                VALUES (%s, %s, %s, %s, %s, NOW())
            """
            cursor.execute(
                sql, (camera_code, event_code, thumb_path_db, full_path_db, boxes)
            )
        ...
        return {
            "ret_code": 0,
            "msg": "Inserted warning via Python API",
            "data": {"id": warning_id},
        }
    except Exception as e:
        return {"ret_code": -1, "msg": f"Database error: {e}"}


# ==========================
# MANAGEMENT APIs (CCTV & LAYOUT)
# ==========================


@app.get("/api/cctv/layout")
async def get_cctv_layout():
    """
    Equivalent to cctvCtrl.php -> getCctvLayout
    """
    try:
        conn = get_db_connection()
        with conn.cursor() as cursor:
            ...
            sql = """
                SELECT 
                    l.id, l.camera_code, l.x_percent, l.y_percent, 
                    l.cam_type, l.view_distance, l.view_angle, l.view_radius,
                    c.ip, c.location as location_json, c.status
                FROM cctv_layout_tbl l
                JOIN cctv_tbl c ON c.code = l.camera_code
                ORDER BY l.camera_code
            """
            cursor.execute(sql)
            rows = cursor.fetchall()
            ...
            # Sensors
            cursor.execute(
                "SELECT device_id, location, x_percent, y_percent, sensor_type FROM sensor_tbl"
            )
            sensors = cursor.fetchall()
        conn.close()

        return {"ret_code": 0, "msg": "OK", "data": rows, "sensors": sensors}
    except Exception as e:
        return {"ret_code": -1, "msg": f"Error: {e}"}


@app.post("/api/cctv/layout")
async def save_cctv_layout(req: dict):
    """
    Equivalent to cctvCtrl.php -> saveCctvLayout (UPSERT)
    """
    items = req.get("items", [])
    if not items:
        return {"ret_code": -1, "msg": "No items to save"}
        ...
        with conn.cursor() as cursor:
            for item in items:
                ...
                if not code:
                    continue
                sql = """
                    INSERT INTO cctv_layout_tbl 
                    (camera_code, x_percent, y_percent, cam_type, view_distance, view_angle, view_radius)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)
                    ON DUPLICATE KEY UPDATE 
                    x_percent=VALUES(x_percent), y_percent=VALUES(y_percent), 
                    cam_type=VALUES(cam_type), view_distance=VALUES(view_distance),
                    view_angle=VALUES(view_angle), view_radius=VALUES(view_radius)
                """
                cursor.execute(
                    sql,
                    (
                        code,
                        item.get("x_percent", 0),
                        item.get("y_percent", 0),
                        item.get("cam_type", "upper"),
                        item.get("view_distance"),
                        item.get("view_angle"),
                        item.get("view_radius"),
                    ),
                )
        conn.commit()
        conn.close()
...
async def delete_cctv_layout(req: dict):
    camera_code = req.get("camera_code")
    if not camera_code:
...
@app.get("/api/cctv/unmapped")
async def get_unmapped_cctv():
    """
    Equivalent to cctvCtrl.php -> getUnmappedCctvForLayout
    """
    try:
        conn = get_db_connection()
        with conn.cursor() as cursor:
            sql = """
                SELECT code, location, status FROM cctv_tbl 
                WHERE code NOT IN (SELECT camera_code FROM cctv_layout_tbl)
                ORDER BY code
            """
            cursor.execute(sql)
            rows = cursor.fetchall()
...
@app.get("/api/cctv/report")
async def get_report(date: str = None):
    """
    Equivalent to cctvCtrl.php -> getReport
    Scans storage and organizes images into timeslots.
    """
    if not date:
        date = time.strftime("%Y%m%d")

    dir_path = os.path.join(STORAGE_PATH, date)
    if not os.path.exists(dir_path):
        return []

    # Get all camera folders in this date directory
    try:
        cam_folders = [
            d
            for d in os.listdir(dir_path)
            if os.path.isdir(os.path.join(dir_path, d))
            and d not in ["thumbshot", "fullshot"]
        ]
    except:
        return []

    # Prepare timeslots (00:00, 00:15, 00:30, 00:45 ...)
    timeslots = []
    for h in range(24):
        for m in ["00", "15", "30", "45"]:
            timeslots.append(f"{h:02d}:{m}")

    result_list = []
    # Key metadata for FE
    conn = get_db_connection()
    with conn.cursor() as cursor:
        cursor.execute("SELECT code, location, area, category FROM cctv_tbl")
        camera_info = {r["code"]: r for r in cursor.fetchall()}
    conn.close()

    for cam_code in cam_folders:
        cam_path = os.path.join(dir_path, cam_code)
        res = {"code": cam_code}
        # Initialize slots
        for ts in timeslots:
            res[ts] = []
        ...
        # Info from DB
        if cam_code in camera_info:
            res["location"] = camera_info[cam_code]["location"]
            res["area"] = camera_info[cam_code]["area"]
            res["category"] = camera_info[cam_code]["category"]

        # Scan files
        for filename in os.listdir(cam_path):
            if filename.endswith(".jpg"):
                # Guess slot from filename (e.g. 081234.jpg)
                # handle "frame_..." prefix if exists
                clean_name = filename.replace("frame_", "").split(".")[0]
                if len(clean_name) >= 4:
                    hh = clean_name[:2]
                    mm = int(clean_name[2:4])
                    if mm < 15:
                        slot = f"{hh}:00"
                    elif mm < 30:
                        slot = f"{hh}:15"
                    ...
                    if slot in res:
                        res[slot].append(f"{date}/{cam_code}/{filename}")
...
# Email Batching (Gộp Mail) - Database-backed Queue
_PENDING_ALERTS = []  # Memory Queue
...
def flush_alerts_loop():
    ...
    print("🚀 Email Batching Thread (VG Sync) started!")
    global _PENDING_ALERTS
    while True:
        try:
            # 1. Chuyển từ bộ nhớ sang Database Queue
            alerts_in_mem = []
            with _ALERTS_LOCK:
                if _PENDING_ALERTS:
            ...
            if alerts_in_mem:
                ...
                try:
                    with conn.cursor() as cur:
                        for alert in alerts_in_mem:
                            cur.execute(
                                "INSERT INTO cctv_alert_queue (camera_code, ip, technical_detail) VALUES (%s, %s, %s)",
                                (
                                    alert["camera_code"],
                                    alert["ip"],
            ...
            # 2. Kiểm tra cooldown 30 phút từ database (system_settings)
            conn = get_db_connection()
            try:
                with conn.cursor() as cur:
                    # Đảm bảo record có sẵn
                    cur.execute(
                        "INSERT IGNORE INTO system_settings (setting_key, setting_value) VALUES ('last_alert_sent_at', '1970-01-01 00:00:00')"
                    )

                    # Lock record để tránh tranh chấp giữa các process
                    cur.execute(
                        "SELECT setting_value FROM system_settings WHERE setting_key = 'last_alert_sent_at' FOR UPDATE"
                    )
                    row = cur.fetchone()
                    last_sent_str = row["setting_value"]
                    last_sent_dt = datetime.fromisoformat(last_sent_str)

                    now = datetime.now()
                    # Cooldown 30 phút
                    if (now - last_sent_dt) > timedelta(minutes=30):
                        # Lấy tất cả thông báo đang đợi trong queue
                        cur.execute(
                            "SELECT id, camera_code, ip, technical_detail FROM cctv_alert_queue ORDER BY id"
                        )
                        queued_alerts = cur.fetchall()

                        if queued_alerts:
                            # Group alerts by camera for cleaner email
                            unique_cams = {}
                            details_text = ""
                            for a in queued_alerts:
                                key = f"{a['camera_code']} ({a['ip']})"
                                unique_cams[key] = True
                                details_text += f"- {key}: {a['technical_detail']}\n"

                            cam_list_str = ", ".join(unique_cams.keys())
                            subject = f"[VG CCTV ALERT] Issues with: {cam_list_str}"
                            body = f"Hệ thống ghi nhận sự cố tại các camera sau đây:\n\n{cam_list_str}\n\n"
                            body += (
                                "--------------------------------------------------\n"
                            )
                            body += f"Chi tiết kỹ thuật / Technical Details:\n{details_text}\n"
                            body += (
                                "--------------------------------------------------\n"
                            )
                            body += (
                                f"Thời gian gửi: {now.strftime('%Y-%m-%d %H:%M:%S')}\n"
                            )
                            body += "Vui lòng kiểm tra tình trạng kết nối mạng hoặc thiết bị."

                            send_alert_email(subject, body)

                            # Cập nhật thời gian gửi và xóa queue
                            cur.execute(
                                "UPDATE system_settings SET setting_value = %s WHERE setting_key = 'last_alert_sent_at'",
                                (now.isoformat(),),
                            )
                            cur.execute("DELETE FROM cctv_alert_queue")
                            conn.commit()
                            print(f"📦 Batched email sent and queue cleared at {now}")
                        else:
                            conn.commit()  # Release lock if no alerts
                    else:
                        conn.commit()  # Not time yet, release lock
            except Exception as e:
            ...
            print(f"❌ Fatal error in flush loop: {e}")
            time.sleep(10)


def check_camera_job():
    """Kiểm tra tình trạng camera (Health Check) và gom vào hàng đợi mail nếu lỗi."""
    print("🔍 Camera Health Check Job (VG Sync) started!")
    # Để đơn giản, ta check mỗi camera cách nhau 15 phút, hoặc check tất cả mỗi 5 phút
    while True:
        try:
            conn = get_db_connection()
            try:
                with conn.cursor() as cur:
                    cur.execute(
                        "SELECT code, ip FROM cctv_tbl WHERE status = 'working' AND ip IS NOT NULL AND ip != ''"
                    )
            ...
            for cam in cameras:
                ...
                # Ping check đơn giản hoặc thử lấy snapshot nhanh
                try:
                    # Timeout 2s để không treo loop
                ...
                if not is_ok:
                    technical_detail = "Connection failed / Request timeout (Cảnh báo từ Health Check Job)"
                    with _ALERTS_LOCK:
                    ...
                    print(f"⚠️ Health Check: Camera {code} ({ip}) is down.")

            # Nghỉ 5 phút giữa các vòng quét sức khỏe
            time.sleep(5 * 60)
        except Exception as e:
...
WARNING_API_URL = os.getenv(
    "WARNING_API_URL",
    f"http://{API_HOST if API_HOST != '0.0.0.0' else 'localhost'}:{API_PORT}/api/cctv/ai/insert_warning",
)
...
PHASH_THRESHOLD = int(os.getenv("PHASH_THRESHOLD", "11"))  # Hamming distance (0-64)
...
def boxes_intersect_or_close(b1, b2, margin_scale=0.15):
    """
    Check if two bounding boxes are overlapping or very close.
    margin_scale: extra margin relative to box size to define "closeness".
    """
    x1_1, y1_1, x2_1, y2_1 = b1["x1"], b1["y1"], b1["x2"], b1["y2"]
    x1_2, y1_2, x2_2, y2_2 = b2["x1"], b2["y1"], b2["x2"], b2["y2"]

    w1, h1 = x2_1 - x1_1, y2_1 - y1_1
    w2, h2 = x2_2 - x1_2, y2_2 - y1_2

    margin_w = min(w1, w2) * margin_scale
    margin_h = min(h1, h2) * margin_scale

    # Check intersection with margin
    if (
        x1_1 - margin_w > x2_2
        or x2_1 + margin_w < x1_2
        or y1_1 - margin_h > y2_2
        or y2_1 + margin_h < y1_2
    ):
        return False
    return True


def get_people_clusters(boxes):
    """
    Groups bounding boxes into clusters based on proximity.
    """
    if not boxes:
        return []
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if boxes_intersect_or_close(boxes[i], boxes[j]):
                union(i, j)
...
def fetch_snapshot_bytes(ip: str) -> bytes:
    ...
    try:
        ...
        if resp.status_code == 401:
            ...
            raise Exception(
                f"Digest failed HTTP {resp2.status_code}, head={resp2.content[:120]}"
            )
...
async def check_phone_parallel(
    ...
    if hits > 0:
        try:
            ...
            print(
                f"[PHONE] ERROR camera={camera_code} stage=send_warning_event err={e}"
            )
...
async def yolo_branch(
    ...
    try:
        ...
        async def check_crowd_parallel() -> Dict[str, Any]:
            ...
            # --- Logic thời gian & phân cụm (Yêu cầu mới) ---
            now_local = datetime.now()
            current_time_str = now_local.strftime("%H:%M")
            # Giờ làm việc: 07:15 - 16:45
            is_working_hours = "07:15" <= current_time_str <= "16:45"

            if is_working_hours:
                # Trong giờ: Kiểm tra tụ tập cụm dựa trên ngưỡng max_people_allowed
                clusters = get_people_clusters(person_boxes)
                max_cluster_size = max([len(c) for c in clusters]) if clusters else 0

                # Cảnh báo nếu có bất kỳ cụm nào vượt quá ngưỡng cho phép
                if max_people_allowed > 0 and max_cluster_size > max_people_allowed:
                    await asyncio.to_thread(
                        send_warning_event, camera_code, "crowb", jpeg_bytes
                    )
                    crowd_result["crowd_alert"] = True
                    result["warnings"].append("crowb")
                    print(
                        f"[CROWD] ALERT (Working Hours) camera={camera_code} max_cluster={max_cluster_size} threshold={max_people_allowed}"
                    )
            else:
                # Ngoài giờ (Sau 16:45 hoặc Trước 07:15): Cứ có người là báo động
                if people_count > 0:
                    await asyncio.to_thread(
                        send_warning_event, camera_code, "crowb2", jpeg_bytes
                    )
                    crowd_result["crowd_alert"] = True
                    result["warnings"].append("crowb")
                    print(
                        f"[CROWD] ALERT (Outside Hours) camera={camera_code} people_count={people_count}"
                    )
        ...
        async def check_intruder_fenceline_parallel() -> Dict[str, Any]:
            ...
            await asyncio.to_thread(
                send_warning_event, camera_code, "intruder", jpeg_bytes
            )
...
async def snapshot_producer_loop():
    ...
    print(
        f"[SNAPSHOT_PRODUCER] capture_max_concurrent={SNAPSHOT_CAPTURE_MAX_CONCURRENT}"
    )
    print(
        f"[SNAPSHOT_PRODUCER] fetch_timeout={SNAPSHOT_FETCH_TIMEOUT_SEC}s cycle_target={TIME_LOOP_ONE_CYCLE}s"
    )
    ...
    while True:
        ...
        try:
            ...
            if file_count >= SNAPSHOT_QUEUE_MAX:
                print(
                    f"[SNAP][{cycle_id}] PAUSE folder_files={file_count} >= {SNAPSHOT_QUEUE_MAX}, sleep {SNAPSHOT_QUEUE_PAUSE_DURATION}s"
                )
            ...
            print(
                f"[SNAP][{cycle_id}] start capture cameras={len(cams)} folder_files={file_count}"
            )
            ...
            print(
                f"[SNAP][{cycle_id}] done ok={ok}/{len(cams)} err={err} dt={dt:.3f}s folder_files={final_count}"
            )
            ...
            print(
                f"[SNAP][{cycle_id}] CYCLE TIMEOUT after {dt:.3f}s (cycle_timeout={SNAPSHOT_CYCLE_TIMEOUT_SEC})"
            )
        except Exception as e:
                    ...
                    if filename.endswith(".jpg") or filename.endswith(".jpeg"):
                        ...
                        if not is_valid_camera(camera_code):
                            try:
                                os.remove(filepath)
                                print(
                                    f"[AI_CONSUMER] Deleted unconfigured camera snapshot: {camera_code}"
                                )
                            except Exception as e:
                                print(f"[AI_CONSUMER] Failed to delete {filepath}: {e}")
                            continue
...
def serve_html():
    ...
    if not os.path.exists("ai_config_VG.html"):
        return HTMLResponse("<h1>Missing ai_config_VG.html</h1>")
    return FileResponse("ai_config_VG.html")
