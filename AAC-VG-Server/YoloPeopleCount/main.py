...
# ==========================
# MAIL CONFIG (VG Factory - Shared with CH)
# ==========================
MAIL_ENABLED = True
...
# Email Batching (Gộp Mail) - Chuyển sang Database (cctv_alert_queue)
_PENDING_ALERTS = []
...
def flush_alerts_loop():
    ...
    print("🚀 Email Batching Thread (VG - Surgical) started!")
    ...
    while True:
        try:
            ...
            # 3. Đã qua 30 phút -> Gửi mail
            conn = get_conn()
            try:
                with conn.cursor() as cur:
                    # Transactional: Khóa DB
                    cur.execute("BEGIN")
                    ...
                    if alerts_to_send:
                        ...
                        subject = f"⚠️ [VG CCTV] Cảnh báo lỗi {count} camera"
...
PHASH_THRESHOLD = int(os.getenv("PHASH_THRESHOLD", "15"))  # Hamming distance (0-64)
...
def get_cameras() -> List[Dict[str, Any]]:
    ...
    """
    sql = """
        SELECT id, code, ip, cctv_url, ai_config, threshold, consecutive_timeouts
        FROM cctv_tbl
        ...
        result.append(
            {
                ...
                "consecutive_timeouts": int(r.get("consecutive_timeouts") or 0),
            }
        )
    return result
...
def save_ai_config(
    ...
    cfg: Dict[str, Any] = {
        ...
        "regions": regions or {},
        "working_hours_start": working_hours_start or "07:15",
        "working_hours_end": working_hours_end or "16:45",
    }
...
async def check_phone_parallel(
    ...
    async def dino_one(idx: int, roi: Image.Image) -> Dict[str, Any]:
        async with DINO_SEMAPHORE:
            t0 = time.time()
            try:
                # Use threshold from top-level ai_config (merged with DB threshold in get_cameras)
    ...
    if hits > 0:
        try:
            ...
            for i, (r, box) in enumerate(zip(dino_results, boxes_sorted)):
                if isinstance(r, dict) and r.get("has_phone"):
                    person_x1, person_y1 = box["x1"], box["y1"]
                    person_w = box["x2"] - box["x1"]
                    person_h = box["y2"] - box["y1"]

                    # Duyệt qua các sub-boxes của phone trong ROI này
                    valid_phone_in_roi = False
                    for pb in r.get("phone_sub_boxes", []):
                        # pb["x1"], etc. are relative to ROI (0 to person_w)
                        # Kiểm tra xem tâm của phone có nằm trong vùng "lõi" của ROI không (tránh mép)
                        # Padding 10% mỗi bên
                        px_center = (pb["x1"] + pb["x2"]) / 2
                        py_center = (pb["y1"] + pb["y2"]) / 2

                        margin_w = person_w * 0.1
                        margin_h = person_h * 0.1

                        if (margin_w <= px_center <= person_w - margin_w) and (
                            margin_h <= py_center <= person_h - margin_h
                        ):
                            valid_phone_in_roi = True
                            break

                    # Nếu hit này là "ngoại lai" (ở mép), bỏ qua để tránh báo nhầm người bên cạnh
                    if not valid_phone_in_roi:
                        print(
                            f"[PHONE] camera={camera_code} IGNORE_HIT roi={i+1} reason=phone_on_edge"
                        )
                        continue
...
async def yolo_branch(
    ...
    try:
        ...
        async def check_crowd_parallel() -> Dict[str, Any]:
            ...
            # --- Logic thời gian & phân cụm (Cấu hình) ---
            ...
            if is_working_hours:
                ...
                # Ngoài giờ: Chỉ báo động nếu detect_off_hours được bật
    ...
    async with SNAPSHOT_CAPTURE_SEMAPHORE:
        try:
            ...
            try:
                with conn.cursor() as cur:
                    cur.execute(
                        "UPDATE cctv_tbl SET cctv_status=%s, consecutive_timeouts=%s, updated_at=CURRENT_TIMESTAMP WHERE code=%s",
                        (new_status, new_timeouts, code),
                    )
            finally:
                conn.close()
            # -------------------------------
            ...
            err_msg = (
                f"Timeout ({SNAPSHOT_FETCH_TIMEOUT_SEC}s)" if is_timeout else str(e)
            )
            print(f"[SNAP][{cycle_id}] {code} ERROR ip={ip} dt={dt:.3f}s err={err_msg}")

            # --- HEALTH UPDATE (FAILURE) ---
            old_timeouts = int(row.get("consecutive_timeouts") or 0)
            new_timeouts = old_timeouts + 1

            # Giống CH: >= 12 lần (tầm 1 phút) thì mới báo lỗi
            if new_timeouts >= 12:
                new_status = "warning"  # Giả định snapshot lỗi là warning (hoặc offline nếu muốn)
            else:
                new_status = row.get("cctv_status") or "online"

            alert_muted = int(row.get("alert_muted") or 0)
            ...
            # Gửi mail nếu đạt ngưỡng (Lần đầu ở 12 check, sau đó nhắc lại mỗi 30 phút = 360 check)
            is_first_alert = new_timeouts == 12
            is_reminder = new_timeouts > 12 and (new_timeouts - 12) % 360 == 0

            if (is_first_alert or is_reminder) and alert_muted == 0:
                alert_info = {
                    "camera_code": code,
                    "ip": ip,
                    "status": new_status,
                ...
                print(f"📝 Alert for {code} added to batch queue.")
            # -------------------------------
...
async def new_background_loop():
    ...
    # Run producer, consumer and email flusher concurrently
    await asyncio.gather(
        snapshot_producer_loop(),
        ai_consumer_loop(),
        asyncio.to_thread(flush_alerts_loop),
        return_exceptions=True,
    )
...
def update_ai_config(payload: Dict[str, Any]):
    # Support both 'camera_code' and 'code'
    cam = (payload.get("camera_code") or payload.get("code") or "").strip()
    if not cam:
        raise HTTPException(status_code=400, detail="camera_code is required")

    # Support nested 'ai_config' or flat fields
    cfg = (
    ...
    try:
        save_ai_config(
            camera_code=cam,
            actions=cfg.get("actions") or {},
            interval_seconds=int(cfg.get("interval_seconds", 5)),
            regions=cfg.get("regions") or {},
            max_people_allowed=cfg.get("max_people_allowed"),
            working_hours_start=cfg.get("working_hours_start"),
            working_hours_end=cfg.get("working_hours_end"),
        )
...
def detect_phone_dino_roi(
    ...
    # Lấy tọa độ tương đối của điện thoại trong ROI
    phone_sub_boxes = []
    if has_phone:
        try:
            ...
            for i, l in enumerate(lbls):
                if "phone" in l.lower():
                    # xyxy relative to ROI image pixels
                    box = [float(v) for v in boxes_raw[i].tolist()]
                    phone_sub_boxes.append(
                        {
                            "x1": box[0],
                            "y1": box[1],
                            "x2": box[2],
    ...
    return {
        ...
        "labels": lbls,
        "phone_sub_boxes": phone_sub_boxes,
    }
