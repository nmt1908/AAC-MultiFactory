...
DEMO_CAMERA_CODE = os.getenv("DEMO_CAMERA_CODE", "B1025_G")
...
# def detect_people_boxes_yolo(
#     image: Image.Image,
#     confidence_threshold: float = 0.25,
# ) -> Dict[str, Any]:
#     """
#     Chạy PersonModel để lấy bbox persons.
#     Return:
#       {
#         "has_person": bool,
#         "boxes": [{"x1":int,"y1":int,"x2":int,"y2":int,"confidence":float}],
...
#     return {"has_person": len(boxes) > 0, "boxes": boxes, "max_confidence": max_conf}
...
#     """
#     Fetch JPEG snapshot từ IP camera.
#     Trả về bytes hoặc raise exception.

#     Improved error handling:
#     - Timeout 3s (giảm từ 5s)
#     - Silent fail cho connection refused (camera offline)
...
#     try:
#         # Try basic auth first (faster)
#         print(f"[fetch_snapshot_bytes] GET {url}")
#         resp = requests.get(
#             url,
#             auth=HTTPBasicAuth(SNAP_USER, SNAP_PASS),
#             timeout=3,  # Giảm từ 5s → 3s
#             stream=False,
#         )

#         if resp.status_code == 200:
#             print(f"[fetch_snapshot_bytes] basic status: {resp.status_code}")
#             return resp.content

#         # If basic fails with 401, try digest
#         if resp.status_code == 401:
#             print(f"[fetch_snapshot_bytes] basic status: {resp.status_code}")
#             resp_digest = requests.get(
#                 url, auth=HTTPDigestAuth(SNAP_USER, SNAP_PASS), timeout=3, stream=False
#             )
#             if resp_digest.status_code == 200:
#                 print(
#                     f"[fetch_snapshot_bytes] digest status: {resp_digest.status_code}"
#                 )
#                 return resp_digest.content

#         # Other errors
#         raise Exception(f"HTTP {resp.status_code}")

#     except requests.exceptions.ConnectionError as e:
#         # Camera offline - SILENT (không log spam)
#         if "Connection refused" in str(e) or "No route to host" in str(e):
#             raise Exception(f"Camera offline: {ip}")
#         # Log other connection errors
#         print(f"fetch_snapshot_bytes error: {e}")
#         raise

#     except requests.exceptions.Timeout:
#         # Timeout - SILENT (camera chậm/offline)
...
async def fire_branch(
    ...
    return result
# ==========================
# QUEUE-BASED SNAPSHOT SYSTEM
# ==========================

# Global queues (initialized in lifespan)
snapshot_queue: asyncio.Queue = None
qwen_semaphore: asyncio.Semaphore = None


# async def capture_one_snapshot(row: Dict[str, Any], now: datetime):
#     """
#     Helper function để lấy snapshot từ 1 camera.
#     Trả về snapshot_item hoặc None nếu skip/error.
#     """
#     code = row.get("code")
#     ip = row.get("ip")
...
#     except Exception as e:
#         print(f"[SNAPSHOT_PRODUCER] {code} parse ai_config error: {e}")
#         return None
...
#     # Capture snapshot continuously - no interval checking
#     # Consumer processes oldest files first, maintaining natural intervals
#     try:
#         t_start = datetime.now(timezone.utc)

#         # Check if demo camera
#         if (
#             DEMO_CAMERA_CODE
#             and code.strip().upper() == DEMO_CAMERA_CODE.strip().upper()
#         ):
#             snap_bytes = await asyncio.to_thread(get_demo_image_bytes)
#         else:
...
#         snap_time = (t_end - t_start).total_seconds()
...
#             "ai_config": cfg,
#             "timestamp": now,
#             "capture_time": snap_time,
#         }

#         return snapshot_item

#     except Exception as e:
#         print(f"[SNAPSHOT_PRODUCER] {code} snapshot error: {e}")
#         return None
...
def preload_all_camera_configs():
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT code, ai_config FROM cctv_tbl WHERE capture_method = 'snap'"
                )
                rows = cur.fetchall()
        finally:
            conn.close()
...
# Cache for working cameras
# _working_snap_cameras = []
# _working_snap_cameras_ts = 0.0


# async def snapshot_producer_loop():
#     """
#     Continuously capture snapshots from all configured cameras.

#     PARALLEL CAPTURE: Lấy snapshot từ tất cả cameras ĐỒNG THỜI trong mỗi vòng.

#     Chỉ save to disk, consumer sẽ tự scan folder và xử lý theo thứ tự thời gian.

#     Flow control:
#     - Pause khi số file trong folder >= SNAPSHOT_QUEUE_MAX (250)
#     - Đợi SNAPSHOT_QUEUE_PAUSE_DURATION (30s)
#     - Chỉ resume khi số file < 60% SNAPSHOT_QUEUE_MAX (150)
#     """
#     print(f"[SNAPSHOT_PRODUCER] Started (PARALLEL MODE)")
#     print(
#         f"[SNAPSHOT_PRODUCER] Mode: CAPTURE → SAVE TO DISK (consumer will scan folder)"
#     )
#     print(
#         f"[SNAPSHOT_PRODUCER] Flow control: pause at {SNAPSHOT_QUEUE_MAX} files, resume below {int(SNAPSHOT_QUEUE_MAX * 0.6)} files (60%)"
#     )
#     print(
#         f"[SNAPSHOT_PRODUCER] Cycle timing: ONE CYCLE every {TIME_LOOP_ONE_CYCLE}s"
#     )
...
#             if loop_count % 10 == 0:  # Every 10 rounds
#                 print(f"[SNAPSHOT_PRODUCER] Heartbeat: {loop_count} cycles completed")
            
#             # Count files in snapshot folder
#             file_count = 0
...
#                 print(
#                     f"[SNAPSHOT_PRODUCER] Folder has {file_count} files (>= {SNAPSHOT_QUEUE_MAX}), pausing for {SNAPSHOT_QUEUE_PAUSE_DURATION}s..."
#                 )
#                 await asyncio.sleep(SNAPSHOT_QUEUE_PAUSE_DURATION)

#                 # Recheck after wait - only resume if below 60% threshold
#                 resume_threshold = int(SNAPSHOT_QUEUE_MAX * 0.6)
#                 new_count = (
#                     len(
#                         [
#                             f
...
#                 if new_count >= resume_threshold:
#                     print(
#                         f"[SNAPSHOT_PRODUCER] Folder still at {new_count} files (>= {resume_threshold}), continuing to wait..."
#                     )
#                     continue  # Continue pausing
#                 else:
#                     print(
#                         f"[SNAPSHOT_PRODUCER] Folder dropped to {new_count} files (< {resume_threshold}), resuming..."
#                     )
...
#             # Capture snapshots for all cameras - PARALLEL!
#             now = datetime.now(timezone.utc)
#             round_start_time = datetime.now(timezone.utc)

#             print(
#                 f"[SNAPSHOT_PRODUCER] Cycle {loop_count}: Starting parallel capture for {len(cams)} cameras..."
#             )

#             tasks = [asyncio.create_task(capture_one_snapshot(row, now)) for row in cams]
#             done, pending = await asyncio.wait(tasks, timeout=5, return_when=asyncio.ALL_COMPLETED)


#             # Cancel pending tasks (cameras quá chậm)
#             for task in pending:
#                 task.cancel()
...
#             )
#             print(
#                 f"[SNAPSHOT_PRODUCER] Cycle {loop_count}: {captured_count}/{len(cams)} captured in {round_time:.3f}s, folder_files={final_file_count}"
#             )

#             # ==== CYCLE TIMING CONTROL ====
#             cycle_elapsed = time.time() - cycle_start_time
#             sleep_time = max(0, TIME_LOOP_ONE_CYCLE - cycle_elapsed)
            
#             if sleep_time > 0:
#                 print(
#                     f"[SNAPSHOT_PRODUCER] Cycle {loop_count} completed in {cycle_elapsed:.2f}s, "
#                     f"sleeping {sleep_time:.2f}s to maintain {TIME_LOOP_ONE_CYCLE}s cycle..."
#                 )
#                 await asyncio.sleep(sleep_time)
#             else:
#                 print(
#                     f"[SNAPSHOT_PRODUCER] ⚠️  WARNING: Cycle {loop_count} took {cycle_elapsed:.2f}s "
#                     f"(exceeds {TIME_LOOP_ONE_CYCLE}s target), starting next cycle immediately"
#                 )

#         except Exception as e:
#             print(f"[SNAPSHOT_PRODUCER] unexpected error: {e}")
#             import traceback
...
#     - Snapshot queue size
#     - Qwen semaphore available slots
#     - Parallel camera semaphore available slots
#     """
#     return {
#         "snapshot_queue_size": snapshot_queue.qsize() if snapshot_queue else 0,
#         "snapshot_queue_max": SNAPSHOT_QUEUE_MAX,
#         "snapshot_queue_resume_threshold": SNAPSHOT_QUEUE_RESUME_THRESHOLD,
#         "qwen_slots_available": qwen_semaphore._value if qwen_semaphore else 0,
#         "qwen_slots_max": QWEN_MAX_CONCURRENT,
#         "parallel_camera_slots_available": PARALLEL_CAMERA_SEMAPHORE._value,
#         "parallel_camera_slots_max": MAX_PARALLEL_CAMERAS,
#     }
...
# async def run_ai_for_image_yolo_only(
#     cam: str, img: Image.Image, cfg: Dict[str, Any]
# ) -> Dict[str, Any]:
#     w, h = img.size
