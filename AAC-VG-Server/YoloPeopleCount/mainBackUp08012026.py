...
async def check_phone_parallel(
    ...
    collage, roi_images = create_roi_collage_from_boxes(img, boxes_for_phone)
    ...
    async def dino_one(idx: int, roi: Image.Image) -> Dict[str, Any]:
        async with DINO_SEMAPHORE:
            t0 = time.time()
            try:
                out = await asyncio.to_thread(detect_phone_dino_roi, roi, 0.3)
    ...
    if hits > 0:
        try:
            await asyncio.to_thread(send_warning_event, camera_code, "smartphone", jpeg_bytes)
            phone_result["phone_alert"] = True
            result["warnings"].append("smartphone")
            print(f"[PHONE] ALERT_SENT camera={camera_code} event=smartphone")
        except Exception as e:
...
def create_roi_collage_from_boxes(img: Image.Image, person_boxes: List[Dict[str, Any]]):
    """
    Returns: (collage_img, roi_images)
    """
    if not person_boxes:
        return None, []
